...
<style>
   ...
   .spkl-header {
      background: linear-gradient(135deg, #c33030, #fa7b58);
      color: white;
      padding: 15px;
      position: relative;
   }
            ...
            <div class="col-md-3">
                              ...
                              <div class="col-md-12">
                                 <div class="form-group form-group-default">
                                    ...
                                    <select class="form-control"  name="cuti_backup" id="cuti_backup">
                                       ...
                                       <option {{$absenceEmp->leader_id == $absenceEmp->cuti_backup_id ? 'selected' : ''}} value="{{$absenceEmp->leader_id}}">{{$absenceEmp->leader->biodata->fullName()}} </option>
                                       @foreach ($emps as $emp)
               ...
               <div class="card spkl-card">
                  ...
                  <div class="spkl-header">
                     <div>
                           <h5 class="mb-1">Detail <x-status.absence-type :absence="$absenceEmp" /> </h5>
                           <div class="spkl-code">
                              <i class="fa fa-barcode"></i> {{$absenceEmp->code}}
                           </div>
                     </div>
                  ...
                  <div class="spkl-body">
                     ...
                     <div class="d-flex justify-content-between align-items-center">
                           ...
                           <div>
                              ...
                              {{-- @if ($absenceEmp->status == 0)
                              <a href="" class="btn mb-2 btn-primary m btn-block" data-target="#modal-release-spkl" data-toggle="modal"><i class="fa fa-paper-plane"></i> Release</a>
                              @endif --}}
                              @if($absenceEmp->status == 0)
                                 @if ($absenceEmp->type == 5 || $absenceEmp->type == 10)
                                    @if ($absenceEmployeeDetails)
                     ...
                     @if ($absenceEmp->status == 101 || $absenceEmp->status == 202 || $absenceEmp->status == 303 )
                        <div class="chat-row mt-2">
                              <div class="chat-bubble chat-left">
                                    <small>{{$absenceEmp->reject_desc}}</small>
                                 <div class="chat-time"> 
                                    @if ($absenceEmp->status == 303)
                              HRD
                                 @else
                                 {{$absenceEmp->rejectBy->biodata->fullName()}}
                              @endif
                                    {{formatDateTime($absenceEmp->reject_date)}}</div>
                              </div>
                        </div>
                     @endif
                  ...
                  <div class="spkl-footer">
                     ...
                     <small class="text-muted">
                        @if ($pageType == 'draft')
                        <a href="{{route('employee.absence.draft')}}" class=""><i class="fa fa-backward"></i> Back</a>
                        @elseif ($pageType == 'index')
                        <a href="{{route('employee.absence')}}" class=""><i class="fa fa-backward"></i> Back</a>
                        @elseif ($pageType == 'progress')
                        <a href="{{route('employee.absence.pending')}}" class=""><i class="fa fa-backward"></i> Back</a>
                        @elseif ($pageType == 'approval')
                        <a href="{{route('leader.absence')}}" class=""><i class="fa fa-backward"></i> Back</a>
                     @endif
               ...
               @if ($absenceEmp->type == 5)
                  @if ($absenceEmp->status == 0)
                     ...
                     <div class="row">
                        <div class="col-md-12">
                           {{-- <small>#Pilih karyawan pengganti</small> --}}
                           <div class="form-group form-group-default">
                              <label>Karyawan Pengganti</label>
                              <select class="form-control"  name="cuti_backup" id="cuti_backup">
                                 <option value="" disabled selected>Select</option>
               ...
               @if ($transfer == 1)
                  <table>
                     <tbody>
                        <thead>
                           <tr>
                              <td colspan="2">
                                 @if ($absenceEmp->additional_id == null)
                                    <a href="#" data-target="#modal-refund-abs-{{$absenceEmp->id}}" data-toggle="modal" class="" >Refund</a>
                                    @else
                                    <a href="#"  class="" >Refund</a>
                                 @endif
                           ...
                           @if ($absenceEmp->additional_id != null)
                              <tr>
                                 <td colspan="2"><i class="fa fa-check text-success"></i> Pengembalian {{formatDate($absenceEmp->additional->date)}}</td>
                              </tr>
                              <tr>
                                 <td colspan="2"></td>
                              </tr>
                  ...
                  <div class="modal fade" id="modal-refund-abs-{{$absenceEmp->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog " role="document">
                           <div class="modal-content">
                              <div class="modal-header">
                                 <h5 class="modal-title" id="exampleModalLabel">Confirmation<br>
                              ...
                              <form action="{{route('employee.absence.refund.abs')}}" method="POST" >
                                 <div class="modal-body">
                                    @csrf
                                    <input type="text" value="{{$absenceEmp->id}}" name="a" id="a" hidden>
                                    {{-- <span>Reject this Form Absence ?</span>
                                    <hr> --}}
                                    <div class="row">
                                       <div class="col-md-6">
                                    ...
                                    <small>Potongan atas tanggal {{formatDate($absenceEmp->date)}} akan dikembalikan pada Periode Gaji Berikutnya (sesuai tanggal yang dipilih)</small> <br>
                                    <br>
                                    <small>Nominal Pengembalian akan otomatis masuk ke Tunjangan Lain-lain <br> (Menu Payroll -> Others)</small>
                                 </div>
                                 <div class="modal-footer">
                                    <button type="button" class="btn btn-light border" data-dismiss="modal">Close</button>
                                    {{-- <a href="{{route('employee.absence.refund.delete', enkripRambo($absenceEmp->id))}}" class="btn btn-light border">Remove</a> --}}
                                    <button type="submit" class="btn btn-danger ">Refund</button>
                                 </div>
                              </form>
                           </div>
   ...
   <div class="modal-dialog " role="document">
      <div class="modal-content text-dark">
         ...
         <form action="{{route('employee.absence.approve.hrd.b', enkripRambo($absenceEmp->id))}}" method="POST" >
            ...
            <div class="modal-footer">
               <button type="button" class="btn btn-light border" data-dismiss="modal">Close</button>
               <button type="submit" class="btn btn-primary">Confirm as HRD</button>
               {{-- <button type="button" class="btn btn-primary ">
                  <a class="text-light" href="{{route('employee.absence.approve.hrd', enkripRambo($absenceEmp->id))}}">Confirm as HRD</a>
               </button> --}}
            </div>
...
{{-- <div class="modal fade" id="modal-add-date" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
   <div class="modal-dialog " role="document">
      <div class="modal-content">
         <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Add Tanggal<br>
