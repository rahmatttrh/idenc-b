...
class AbsenceLeaderController extends Controller
{
   ...
   public function monitoringSpklHrd(){
    ...
    public function historyHrd(){
      // $employee = Employee::where('nik', auth()->user()->username)->first();
      $reqForms = AbsenceEmployee::whereIn('status', [5])->orderBy('release_date', 'desc')->get();
      $activeTab = 'history';
      ...
      if (auth()->user()->hasRole('HRD-KJ12')) {
         $employees = Employee::join('contracts', 'employees.contract_id', '=', 'contracts.id')
            ...
            $reqForms = AbsenceEmployee::whereIn('status', [5])->whereIn('employee_id', $idEmp)->orderBy('release_date', 'desc')->get();
